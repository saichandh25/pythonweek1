...
#creating network 
model = Sequential()
model.add(Dense(512, activation='relu', input_shape=(dimData,)))      #adding hidden layers with different non linear activation functions
model.add(Dense(512, activation='relu'))
model.add(Dense(10, activation='softmax'))
#compiling to produce nueral network
...
plt.title('accuracy of test&train data')
plt.ylabel('accuracy')
plt.xlabel('epoch')
...
plt.title('loss of test&train data')
plt.ylabel('loss')
plt.xlabel('epoch')
plt.legend(loc='upper right')
plt.show()